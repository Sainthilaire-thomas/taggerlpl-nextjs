...
CREATE TABLE public.h2_analysis_pairs (
  pair_id bigint NOT NULL DEFAULT nextval('h2_analysis_pairs_pair_id_seq'::regclass),
  ...
  computation_status text DEFAULT 'pending'::text CHECK (computation_status = ANY (ARRAY['pending'::text, 'computed'::text, 'error'::text])),
  ...
  prev4_speaker text,
  prev4_tag text,
  prev4_start_time double precision,
  prev4_end_time double precision,
  ...
  next4_turn_id integer,
  next4_verbatim text,
  next4_speaker text,
  next4_tag text,
  next4_start_time double precision,
  next4_end_time double precision,
...
CREATE TABLE public.turntagged_backup_20250115 (
  id integer,
  call_id integer,
