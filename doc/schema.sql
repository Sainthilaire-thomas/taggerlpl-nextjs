...
CREATE TABLE public.h2_analysis_pairs (
  ...
  computation_status text DEFAULT 'pending'::text CHECK (computation_status = ANY (ARRAY['computed'::text, 'error'::text, 'pending'::text])),
  ...
  CONSTRAINT fk_conseiller_turn FOREIGN KEY (conseiller_turn_id) REFERENCES public.turntagged(id),
  CONSTRAINT fk_client_turn FOREIGN KEY (client_turn_id) REFERENCES public.turntagged(id)
);
