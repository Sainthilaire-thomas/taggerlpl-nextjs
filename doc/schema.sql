...
CREATE TABLE public.analysis_pairs (
  ...
  strategy_family text NOT NULL CHECK (strategy_family = ANY (ARRAY['ENGAGEMENT'::text, 'OUVERTURE'::text, 'REFLET'::text, 'EXPLICATION'::text])),
  ...
  reaction_tag text NOT NULL CHECK (reaction_tag = ANY (ARRAY['CLIENT_POSITIF'::text, 'CLIENT_NEUTRE'::text, 'CLIENT_NEGATIF'::text])),
  ...
  level0_annotator_agreement numeric,
  level0_validated_at timestamp without time zone,
  level0_validated_by text,
  level0_notes text,
  ...
  m1_computed_at timestamp without time zone,
  ...
  m2_computed_at timestamp without time zone,
  ...
  m3_computed_at timestamp without time zone,
  computation_status text DEFAULT 'pending'::text CHECK (computation_status = ANY (ARRAY['pending'::text, 'partial'::text, 'complete'::text, 'error'::text])),
...
CREATE TABLE public.h2_analysis_pairs_backup_20251120 (
  pair_id bigint,
  call_id text,
  conseiller_turn_id integer,
  ...
  client_verbatim text,
  client_speaker text,
  client_start_time double precision,
  client_end_time double precision,
  prev2_verbatim text,
  prev2_speaker text,
  prev2_tag text,
  prev2_start_time double precision,
