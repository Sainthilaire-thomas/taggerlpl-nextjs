...
export class OpenAI3TXClassifier implements UniversalAlgorithm {
  ...
  describe(): AlgorithmDescriptor {
    return {
      ...
      displayName: "OpenAI ÔÇö X (3 tours)",
      version: "1.0.0",
      type: "llm",
      target: "X",
      batchSupported: true,
      requiresContext: true, // Ô£à Indique que cet algo n├®cessite du contexte
      description:
      ...
      parameters: {
        model: {
          ...
          description: "Mod├¿le OpenAI ├á utiliser",
        },
        temperature: {
          type: "number",
          min: 0,
  ...
  private async runClientSide(
    ...
    try {
      console.log("­ƒîÉ OpenAI 3T Classification - Appel via API interne client");
  ...
  private async runServerSide(
    ...
    console.log("­ƒöº OpenAI 3T Classification - Appel direct serveur");
...
- REFLET_ACQ: acquiescement court ("oui", "d'accord", "ok", Ôëñ 20 caract├¿res)
...
**Utilise le contexte T-2/T-1 pour d├®sambigu├»ser les cas limites.**
R├®ponds ${
    ...
    const t2 = context.tMinus2?.trim() || "ÔÇö";
    const t1 = context.tMinus1?.trim() || "ÔÇö";
  ...
  private applyContextualRules(
    ...
    if (
      t1.includes("client:") &&
      (t1.includes("?") || t1.includes("comment") || t1.includes("o├╣")) &&
  ...
  async batchRun(inputs: unknown[]): Promise<UniversalResult[]> {
    console.log(`­ƒöä Batch processing ${inputs.length} items with context`);
