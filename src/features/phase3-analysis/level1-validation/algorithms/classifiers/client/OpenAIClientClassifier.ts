...
type OpenAIConfig = {
  apiKey?: string; // MAJ ├á chaud possible via updateConfig()
  model?: string;
  temperature?: number;
  maxTokens?: number;
  timeout?: number;
  enableFallback?: boolean; // en cas d'erreur ÔåÆ AUTRE_NON_RECONNU
};
...
export class OpenAIXClassifier implements UniversalAlgorithm {
  ...
  constructor(config: OpenAIConfig = {}) {
    // ÔÜá´©Å ne jamais mettre NEXT_PUBLIC_* ici : la cl├® doit rester c├┤t├® serveur
    this.apiKey = config.apiKey || process.env.OPENAI_API_KEY || "";

    this.config = {
      model: "gpt-4o", // Plus performant que gpt-4o-mini pour la classification complexe
      temperature: 0, // Maintenir ├á 0 pour la coh├®rence
  ...
  describe(): AlgorithmDescriptor {
    return {
      ...
      displayName: "OpenAI ÔÇô X (conseiller)",
  ...
  async run(input: unknown): Promise<UniversalResult> {
    ...
    // ÔöÇÔöÇ LOGIQUE CORRIG├ëE : D├®tection de l'environnement
    const isServer = typeof window === "undefined";

    if (isServer) {
      // ÔöÇÔöÇ Chemin serveur : appel direct OpenAI
      return this.runServerSide(verbatim, startTime);
    } else {
      // ÔöÇÔöÇ Chemin navigateur : passage par l'API interne Next.js
  ...
  private async runClientSide(
    ...
    try {
      console.log("­ƒîÉ OpenAI Classification - Appel via API interne client");
      ...
      if (!response.ok) {
        ...
        console.error(`ÔØî API Error ${response.status}:`, errorText);
  ...
  private async runServerSide(
    ...
    console.log("­ƒöº OpenAI Classification - Appel direct serveur");

    if (!this.apiKey) {
      console.error("ÔØî No OpenAI API key available");
      return this.naFallback(startTime, "no-api-key");
    }

    const controller = new AbortController();
    const timer = setTimeout(() => {
      console.warn("ÔÅ░ OpenAI API timeout");
      controller.abort();
    }, this.config.timeout);

    try {
      console.log(`­ƒñû Calling OpenAI API with model: ${this.config.model}`);
      ...
      const response = await fetch(
        "https://api.openai.com/v1/chat/completions",
        {
          ...
          body: JSON.stringify({
            ...
            // ­ƒöÆ Sortie structur├®e : un JSON strict { "label": <enum> }
            response_format: {
              type: "json_schema",
              json_schema: {
                name: "x_label",
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.error("ÔØî OpenAI API Error:", errorData);
        } catch {}
        return this.naFallback(startTime, reason);
      }

      const data = await response.json();
      console.log("Ô£à OpenAI API Response:", data);

      return this.parseOpenAIResponse(data, startTime);
    } catch (error: any) {
      clearTimeout(timer);
      console.error("ÔØî OpenAI API call failed:", error);
...
R├êGLE FONDAMENTALE - HI├ëRARCHIE DE PRIORIT├ë :
...
CAT├ëGORIES ET MARQUEURS PRIORITAIRES :

­ƒÄ» ENGAGEMENT (PRIORIT├ë 1) - Action du conseiller
...
- "D'accord, je vais v├®rifier votre dossier" ÔåÆ ENGAGEMENT (action prime sur acquiescement)
- "Je comprends, je transf├¿re maintenant" ÔåÆ ENGAGEMENT (action prime sur empathie)

­ƒÄ» OUVERTURE (PRIORIT├ë 2) - Action du client  
...
- "Vous pouvez aller sur le site parce que..." ÔåÆ OUVERTURE (instruction prime sur explication)
- "Il faut pr├®ciser l'heure et la station" ÔåÆ OUVERTURE

­ƒÄ» REFLET (PRIORIT├ë 3) - Reformulation/Acquiescement
...
- REFLET_ACQ : micro-tours Ôëñ20 chars ("oui", "d'accord", "ok")
EXCLUSIONS : si donn├®es chiffr├®es ou marqueurs instruction/explication ÔåÆ pas REFLET

­ƒÄ» EXPLICATION (PRIORIT├ë 4) - Justification/Proc├®dure
MARQUEURS : "parce que", "car", "le syst├¿me fonctionne", "notre politique", "c'est pour ├ºa que"
LOGIQUE : Justification institutionnelle sans action concr├¿te
- "Notre syst├¿me fonctionne en trois ├®tapes" ÔåÆ EXPLICATION
- "C'est normal/faux" (correction normative) ÔåÆ EXPLICATION

R├êGLES DE D├ëPARTAGE CRITIQUES :
- Action pr├®sente ÔåÆ ENGAGEMENT/OUVERTURE m├¬me si acquiescement en d├®but
- Instruction client ÔåÆ OUVERTURE m├¬me si justification apr├¿s
- Donn├®es chiffr├®es/quantifications ÔåÆ EXPLICATION (pas REFLET)
- Micro-tour seul ÔåÆ REFLET_ACQ, mais si suivi d'instruction ÔåÆ prendre l'ensemble`,
      },
      ...
      // Cas pi├¿ge REFLET
      { role: "user", content: "Je vois que vous avez d├®j├á appel├® hier" },
      { role: "assistant", content: '{"label": "REFLET_VOUS"}' },

      { role: "user", content: "Je vois que vous avez re├ºu 1504,29 Ôé¼" },
  ...
  async batchRun(inputs: unknown[]): Promise<UniversalResult[]> {
    console.log(`­ƒöä Batch processing ${inputs.length} items`);
  ...
  private naFallback(startTime: number, reason: string): UniversalResult {
    ...
    console.warn(`ÔÜá´©Å Fallback triggered: ${reason}`);
